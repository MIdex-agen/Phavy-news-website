...
function fetchNews(isSearching){
    ...
    if (isSearching){
        ...
        url = `http://newsapi.org/v2/everything?q=${keyword}&apikey=${API_KEY}&page=${currentPage}`;
    }else{
        const category = currentCategory || document.getElementById('category').value;
        url = `http://newsapi.org/v2/top-headlines?country=us&category=${category}&apikey=${API_KEY}&page=${currentPage}`;
